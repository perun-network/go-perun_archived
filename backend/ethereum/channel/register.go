...
func (a *Adjudicator) registerNonFinal(ctx context.Context, req channel.AdjudicatorReq) error {
	// disputeState, _ := a.contract.Disputes(NewCallOpts(ctx), req.Tx.ID)
	// log.Warnf("version_onchain = %d", disputeState.Version)
	// log.Warnf("version_local = %d", req.Tx.Version)
	return a.callRegister(ctx, req)
}
