...
func (this *DatabaseTest) Has(key string) bool {
	...
	if err != nil {
		this.Fatalf("Has(): Failed to query '%s': %v\n", key, err)
	}
	return has
}
...
func (this *DatabaseTest) MustHave(key string) {
	if !this.Has(key) {
		this.Errorf("Database does not have entry '%s' but it should.\n", key)
	}
	return
}
...
func (this *DatabaseTest) MustNotHave(key string) {
	if this.Has(key) {
		this.Errorf("Database has entry '%s' but it shouldn't.\n", key)
	}
	return
}
...
func (this *DatabaseTest) PutBytes(key string, value []byte) {
	if err := this.Database.PutBytes(key, value); err != nil {
		this.Fatalf("PutBytes(): Failed to put ['%s']='%s'.\n", key, value)
	}
	return
}
...
func (this *DatabaseTest) GetBytes(key string) []byte {
	...
	if err != nil {
		this.Fatalf("Failed to get bytes ['%s']\n", key)
	}
	return value
}
...
func (this *DatabaseTest) MustFailGet(key string) {
	...
	if err == nil {
		this.Errorf("Get() did not fail when expected to (['%s']).\n", key)
	}
	return
}
...
func (this *DatabaseTest) MustGetEqual(key string, expected string) {
	if value := this.Get(key); value != expected {
		this.Errorf(
			"Get() returned the wrong value: ['%s'] (is '%s', expected '%s')\n",
			key,
			value,
			expected)
	}
